...
def build_multi_head_crnn(input_shape, num_classes=3):
    """Builds a more memory-efficient CRNN model."""
    inputs = layers.Input(shape=input_shape)
    
    # --- CNN Feature Extraction Body ---
    ...
    # Add a final pooling layer to shrink the feature map before the LSTM.
    # This pools BOTH frequency and time slightly.
    x = layers.MaxPooling2D(pool_size=(2, 2))(x) 

    # --- Bridge from CNN to LSTM ---
    x = layers.Permute((2, 1, 3))(x) 
    x = layers.Reshape((-1, x.shape[2] * x.shape[3]))(x)

    # --- LSTM Temporal Modeling ---
    # We can also reduce the LSTM units slightly to save memory
    x = layers.Bidirectional(layers.LSTM(96, return_sequences=False))(x) # Reduced from 128 to 96
    ...
    for i in range(4):
        head = layers.Dense(128, activation='relu')(x)
    ...
    model = models.Model(inputs=inputs, outputs=output_heads, name="uav_fault_crnn_efficient")
    return model
...
def main():
    ...
    model = build_multi_head_crnn(input_shape)
